...
//import shapeless3.*
//import shapeless3.labelled.{field, FieldType}
...
trait DerivedDecoders extends PrimitiveDecoders {
  given derive [P <: Product](using m: Mirror.ProductOf[P], ts: JobDecoder[m.MirroredElemTypes]): DerivedJobDecoder[P] =
    new DerivedJobDecoder[P] {
      override private[scheduler] def read(path: Chain[String], data: PartiallyDecodedJobData): Either[Throwable, P] =
        ts.read(path, data).map(m.fromProduct)
    }
